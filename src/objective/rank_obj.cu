...
struct DeviceNDCGCache {
  ...
  dh::device_vector<double> inv_IDCG;  // NOLINT
  std::size_t truncation{0};

  dh::device_vector<std::size_t> sorted_idx_cache;  // avoid allocating memory for each iter.

  dh::device_vector<std::size_t> threads_group_ptr;
  dh::device_vector<bst_group_t> group_ptr;
  std::size_t n_threads{0};

  explicit DeviceNDCGCache(Context const* ctx, MetaInfo const *info, std::size_t ndcg_truncation,
                           common::Span<float const> predts)
      : p_info{info}, truncation{ndcg_truncation} {
    auto labels = info->labels.View(dh::CurrentDevice()).Slice(linalg::All(), 0);
    ...
    CalcQueriesInvIDCG(ctx, labels, d_group_ptr, dh::ToSpan(inv_IDCG), ndcg_truncation);
  ...
  std::size_t Groups() const { return group_ptr.size() - 1; }
  std::size_t Threads() const { return n_threads; }
  common::Span<std::size_t const> ThreadsGroupPtr() const {
  ...
  common::Span<double const> InvIDCG() const { return {inv_IDCG.data().get(), inv_IDCG.size()}; }
...
void LambdaMARTGetGradientNDCGGPUKernel(Context const *ctx,
                                        const HostDeviceVector<bst_float> &preds,
                                        const MetaInfo &info, std::size_t ndcg_truncation,
                                        std::shared_ptr<DeviceNDCGCache> *cache,
                                        HostDeviceVector<GradientPair> *out_gpair) {
  int32_t device_id = ctx->gpu_id;
  dh::safe_cuda(cudaSetDevice(device_id));
  std::size_t n_groups = info.group_ptr_.size() - 1;
  ...
  dh::LaunchN(gpairs.size(), [=] XGBOOST_DEVICE(std::size_t idx) { gpairs[idx] = GradientPair{}; });

  auto &p_cache = *cache;
  if (!p_cache || p_cache->p_info != &info || p_cache->truncation != ndcg_truncation) {
    p_cache = std::make_shared<DeviceNDCGCache>(ctx, &info, ndcg_truncation, predts);
  }
  ...
  dh::LaunchN(p_cache->Threads(), [=] XGBOOST_DEVICE(std::size_t idx) {
    ...
    std::size_t n_data = d_group_ptr[query_group_idx + 1] - data_group_begin;

    auto inv_IDCG = d_inv_IDCG[query_group_idx];

    auto g_labels = labels.Slice(
        linalg::Range(static_cast<std::size_t>(data_group_begin), data_group_begin + n_data));
  ...
  if (!info.weights_.Empty()) {
    ...
    dh::LaunchN(info.num_row_, [=] XGBOOST_DEVICE(std::size_t idx) {
