...
struct ScanOp : public thrust::binary_function<ItemTy, ItemTy, ItemTy> {
  ...
  XGBOOST_DEVICE thrust::tuple<float, size_t>
  DoIt(EvaluateSplitInputs<GradientSumT> input, size_t idx, size_t ridx,
       float l_loss_chg, GradientSumT l_gpair, GradientSumT r_gpair,
    ...
    if (l_loss_chg < loss_chg) {
      return thrust::make_tuple(loss_chg, ridx);
    }
    return thrust::make_tuple(l_loss_chg, idx);
  }
  ...
  XGBOOST_DEVICE ScanElem<GradientSumT> Scan(ScanElem<GradientSumT> const &l, ScanElem<GradientSumT> const &r) const {
    ...
    if (l.idx < left.gradient_histogram.size()) {
      ...
      if (common::IsCat(left.feature_types, l_fidx)) {
        ...
        float loss_chg;
        size_t max_idx;
        thrust::tie(loss_chg, max_idx) = DoIt<forward, true>(
            left, l.idx, r.idx, l.loss_chg, l_gpair, r_gpair, l_fidx);
        return {r_idx, r_gpair, loss_chg, max_idx};
      } else {
        auto l_gpair = l.grad;
        auto r_gpair = left.parent_sum - l_gpair;
        float loss_chg;
        size_t max_idx;
        thrust::tie(loss_chg, max_idx) = DoIt<forward, false>(
            left, l.idx, r.idx, l.loss_chg, l_gpair, r_gpair, l_fidx);
        ret.idx = r_idx;
        ret.grad = l_gpair + r.grad;
        ret.loss_chg = loss_chg;
        ret.max_idx = max_idx;
        return ret;
      }
    } else {
      ...
      if ((right.feature_set.size() != right.feature_segments.size()) &&
        ...
        // column sampling
        return ScanElem<GradientSumT>{r_idx, r.grad, DeviceSplitCandidate{}};
      }
      ...
      if (common::IsCat(right.feature_types, l_fidx)) {
        ...
        auto best = DoIt<forward, true>(right, l_idx, r.idx, l_gpair, r_gpair, l_fidx);
        return {r_idx, r_gpair, best};
      } else {
        auto l_gpair = l.grad;
        auto r_gpair = right.parent_sum - l_gpair;
        auto best = DoIt<forward, false>(right, l_idx, r.idx, l_gpair, r_gpair, l_fidx);
        return ScanElem<GradientSumT>{r_idx, l.grad + r.grad, best};
      }
    }
  }
...
struct WriteScan {
  ...
  XGBOOST_DEVICE void DoIt(ScanElemTy const& candidate) {
    ...
    if (forward) {
      ...
      if (end_idx == idx) {
        printf("writing to: %lu, idx: %lu, loss_chg: %f \n",
               offset + size_t(fidx), candidate.idx,
               candidate.candidate.loss_chg);
...
void EvaluateSplits(common::Span<DeviceSplitCandidate> out_splits,
  ...
  //     std::cout << "i: " << i << ", idx: " << fw.idx << ", idx: " << bw.idx
  //               << ", grad: " << fw.grad << "\n"
  ...
  auto reduce_val = dh::MakeTransformIterator<DeviceSplitCandidate>(
      ...
      [d_out_scan] __device__(size_t idx) {
        ...
        ScanElem<GradientSumT> item = d_out_scan[idx];
        DeviceSplitCandidate candidate;
        // candidate.Update(item.loss_chg, kLeftDir, item.idx);
        return candidate;
      });
