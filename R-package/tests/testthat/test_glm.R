...
test_that("gblinear works", {
  ...
  param <- list(objective = "binary:logistic", eval_metric = "error", booster = "gblinear",
                nthread = 2, eta = 0.8, alpha = 0.0001, lambda = 0.0001, verbosity = VERB)
  ...
  bst <- xgb.train(param, dtrain, n, watchlist, feature_selector = 'shuffle')
  ypred <- predict(bst, dtest)
  expect_equal(length(getinfo(dtest, 'label')), 1611)
  expect_lt(bst$evaluation_log$eval_error[n], ERR_UL)

  bst <- xgb.train(param, dtrain, n, watchlist, feature_selector = 'cyclic',
  ...
  bst <- xgb.train(param, dtrain, 2, watchlist, feature_selector = 'greedy')
  expect_lt(bst$evaluation_log$eval_error[2], ERR_UL)

  bst <- xgb.train(param, dtrain, n, watchlist, feature_selector = 'thrifty',
