...
struct GPUHistMakerDevice {
 ...
 public:
  ...
  GPUHistMakerDevice(Context const* ctx, common::Span<FeatureType const> _feature_types,
                     TrainParam _param, std::shared_ptr<common::ColumnSampler> column_sampler,
                     BatchParam batch_param, MetaInfo const& info, std::vector<bst_idx_t> batch_ptr,
      ...
      : evaluator_{_param, static_cast<bst_feature_t>(info.num_col_), ctx->Device()},
        ...
        cuts_{std::move(cuts)} {
    bool concat_pages{false};  // FIXME;
    this->sampler = std::make_unique<GradientBasedSampler>(
        ctx, info_.num_row_, batch_param, param.subsample, param.sampling_method, concat_pages);
  ...
  DMatrix* Reset(HostDeviceVector<GradientPair>* dh_gpair, DMatrix* p_fmat) {
    ...
    for (std::int32_t k = 0; k < p_fmat->NumBatches(); ++k) {
      ...
      partitioners_.back()->Reset(ctx_, n_samples, this->batch_ptr_[k]);
    }
  ...
  void PartitionAndBuildHist(DMatrix* p_fmat, std::vector<GPUExpandEntry> const& expand_set,
    ...
    if (!candidates.empty()) {
      this->AllReduceHist(build_nidx.at(0), build_nidx.size());
      this->SubtractHist(p_fmat, candidates, build_nidx, subtraction_nidx);
    }
  ...
  void FinalisePosition(RegTree const* p_tree, DMatrix* p_fmat, ObjInfo task,
    ...
    auto it =
        common::MakeIndexTransformIter([this](auto i) { return this->partitioners_[i]->Size(); });
    if (p_fmat->Info().num_row_ != std::accumulate(it, it + this->partitioners_.size(), 0ul)) {
  ...
  void SubtractHist(DMatrix* p_fmat, std::vector<GPUExpandEntry> const& candidates,
...
class GPUHistMaker : public TreeUpdater {
 ...
 public:
  ...
  void InitDataOnce(TrainParam const* param, DMatrix* dmat) {
    ...
    for (auto const& page : dmat->GetBatches<EllpackPage>(ctx_, HistBatch(*param))) {
    ...
    this->maker = std::make_unique<GPUHistMakerDevice>(ctx_, info_->feature_types.ConstDeviceSpan(),
                                                       *param, column_sampler_, HistBatch(*param),
                                                       dmat->Info(), batch_ptr, cuts_);
...
class GPUGlobalApproxMaker : public TreeUpdater {
 public:
  ...
  void Update(TrainParam const* param, linalg::Matrix<GradientPair>* gpair, DMatrix* p_fmat,
    ...
    this->maker_ = std::make_unique<GPUHistMakerDevice>(ctx_, info.feature_types.ConstDeviceSpan(),
                                                        *param, column_sampler_, batch,
