...
class CudfAdapter : public detail::SingleBatchDataIter<CudfAdapterBatch> {
 public:
  ...
  [[nodiscard]] enc::DeviceColumnsView Cats() const {
    common::Span<std::int32_t const> segs{thrust::raw_pointer_cast(this->cat_segments_.data()),
                                          this->cat_segments_.size()};
    return {common::Span{this->cats_}, segs, this->n_total_cats_};
  }
