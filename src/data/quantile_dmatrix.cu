...
void MakeSketches(Context const* ctx,
  ...
  std::unique_ptr<common::SketchContainer> final_sketch;
  ...
  do {
    ...
    if (!ref) {
      if (!final_sketch) {
        final_sketch = std::make_unique<common::SketchContainer>(
      ...
      cuda_impl::Dispatch(proxy, [&](auto const& value) {
        common::AdapterDeviceSketch(value, p.max_bin, proxy->Info(), missing, final_sketch.get());
      });
    }
