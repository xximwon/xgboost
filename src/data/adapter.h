...
[[nodiscard]] std::size_t GetArrowDictionary(Json jcol,
  ...
  n_bytes_ += codes.ElementSize() * codes.Shape<0>();
  n_bytes_ += names.SizeBytes();
...
[[nodiscard]] std::size_t GetArrowNumericIndex(Object::Map const& first,
                                               std::vector<CategoricalIndex>* p_cat_columns) {
  auto const& jindex = get<Object const>(first.at("values"));
  auto index = ArrayInterface<1>{jindex};
  DispatchDType(index, DeviceOrd::CUDA(0), [p_cat_columns](auto t) {
    using T = typename decltype(t)::value_type;
    constexpr bool kKnownType = std::is_same_v<T, std::int32_t>;
...
class ColumnarAdapter : public detail::SingleBatchDataIter<ColumnarAdapterBatch> {
  std::vector<ArrayInterface<1>> columns_;  // fixme: optional allow missing perhaps?
