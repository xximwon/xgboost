...
void RankingCache::InitOnCUDA(Context const* ctx, MetaInfo const& info) {
  ...
  group_ptr_.SetDevice(ctx->DeviceType());
  ...
  threads_group_ptr_.SetDevice(ctx->DeviceType());
  ...
  sorted_idx_cache_.SetDevice(ctx->DeviceType());
...
void NDCGCache::InitOnCUDA(Context const* ctx, MetaInfo const& info) {
  CUDAContext const* cuctx = ctx->CUDACtx();
  auto labels = info.labels.View(ctx->DeviceType()).Slice(linalg::All(), 0);
...
void PreCache::InitOnCUDA(Context const* ctx, MetaInfo const& info) {
  auto const d_label = info.labels.View(ctx->DeviceType()).Slice(linalg::All(), 0);
