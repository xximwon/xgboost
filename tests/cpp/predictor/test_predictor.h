...
void TestPredictionFromGradientIndex(std::string name, size_t rows, size_t cols,
...

  {
    ...
    predictor->InitOutPredictions(&ctx, p_hist->Info(), &approx_out_predictions.predictions, model);
    predictor->PredictBatch(p_hist.get(), &approx_out_predictions, model, 0);

    PredictionCacheEntry precise_out_predictions;
    predictor->InitOutPredictions(&ctx, p_precise->Info(), &precise_out_predictions.predictions,
                                  model);
...

  {
    ...
    PredictionCacheEntry precise_out_predictions;
    predictor->InitOutPredictions(&ctx, p_dmat->Info(), &precise_out_predictions.predictions,
                                  model);
