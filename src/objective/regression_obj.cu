...
class MeanAbsoluteError : public ObjFunction {
 public:
  ...
  void UpdateTreeLeaf(HostDeviceVector<bst_node_t> const& position, MetaInfo const& info,
    ...
    HostDeviceVector<float> alphas{0.5f};
    alphas.SetDevice(ctx_->gpu_id);
    // fixme: use iterator instead.
    ::xgboost::obj::UpdateTreeLeaf(
        ctx_, position, group_idx, info, learning_rate, prediction,
        ctx_->IsCPU() ? alphas.ConstHostSpan() : alphas.ConstDeviceSpan(), p_tree);
  }
