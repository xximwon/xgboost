...
test_that("updating the model works", {
  ...
  # no-subsampling
  p1 <- list(
      objective = "binary:logistic", max_depth = 2, eta = 0.05, nthread = 2, verbosity = 0
  )
  set.seed(11)
  bst1 <- xgb.train(p1, dtrain, nrounds = 10, watchlist)
  tr1 <- xgb.model.dt.tree(model = bst1)

  # with subsampling
  p2 <- modifyList(p1, list(subsample = 0.1))
  set.seed(11)
  bst2 <- xgb.train(p2, dtrain, nrounds = 10, watchlist)
  ...
  set.seed(11)
  bst1r <- xgb.train(p1r, dtrain, nrounds = 10, watchlist)
  ...
  set.seed(11)
  bst2r <- xgb.train(p2r, dtrain, nrounds = 10, watchlist)
  ...
  bst1u <- xgb.train(p1u, dtrain, nrounds = 10, watchlist, xgb_model = bst1)
  ...
  bst2u <- xgb.train(p2u, dtrain, nrounds = 10, watchlist, xgb_model = bst2)
  ...
  bst1ut <- xgb.train(p1ut, dtest, nrounds = 10, watchlist, xgb_model = bst1)
...
test_that("updating works for multiclass & multitree", {
  ...
  p0 <- list(max_depth = 2, eta = 0.5, nthread = 2, subsample = 0.6,
             ...
             base_score = 0, verbosity = 0)
  set.seed(121)
  bst0 <- xgb.train(p0, dtr, 5, watchlist)
  ...
  bst0u <- xgb.train(p0u, dtr, nrounds = bst0$niter, watchlist, xgb_model = bst0)
